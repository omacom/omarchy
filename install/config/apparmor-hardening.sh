...
sudo rm -f /etc/apparmor.d/usr.sbin.sudo /etc/apparmor.d/usr.bin.sudo 2>/dev/null

sudo tee /etc/apparmor.d/usr.bin.sshd >/dev/null <<'EOF'
...
sudo tee /etc/apparmor.d/usr.bin.useradd >/dev/null <<'EOF'
#include <tunables/global>
...
if pacman -Q apparmor &>/dev/null; then
    ...
    sudo aa-enforce /usr/bin/sshd 2>/dev/null
    sudo aa-enforce /usr/bin/useradd 2>/dev/null
