...
# Do not pass `silent` on preauth: when faillock has locked the account,
# pam_faillock must be able to say so. With silent, sudo and the lock screen
# keep reporting a wrong password even for the correct one (#13185).
sed -i 's|^\(auth\s\+required\s\+pam_faillock.so\)\s\+preauth.*$|\1 preauth deny=10 unlock_time=120|' \
