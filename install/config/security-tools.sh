SECURITY_PKGS=(usbguard apparmor aide fail2ban lynis rkhunter clamav)
...
if pacman -Q clamav &>/dev/null; then
    sudo systemctl enable clamav-freshclam.service 2>/dev/null
    sudo systemctl enable clamav-daemon.service 2>/dev/null
...
if command -v lynis &>/dev/null; then
    lynis --cron system >> /var/log/lynis-audit.log 2>&1
fi
...
if command -v freshclam &>/dev/null; then
    freshclam >> /var/log/clamav-update.log 2>&1
fi
AUDITEOF
