...
Omarchy's Cua install also installs the `cua-hyprland-plugin` package when the package channel carries it. Installing it loads nothing: the module sits at `/usr/lib/cua/hyprland/cua-hyprland-plugin.so` until `omarchy-toggle-cua-input` (Trigger > Toggle > Cua Input) runs the package's compatibility check, loads it, and turns on its input route. This requires plugin 0.26.1-4 or newer: its background agent keyboards have independent US keymaps, and the user's layout and remaps stay unchanged. Plain pointer operations do not require a US layout. Foreground keys still depend on the user's mapping and can refuse operations whose key or modifiers have a different meaning; this is not arbitrary-layout or Unicode/IME support. `omarchy-toggle-cua-input --status` reports enabled, loaded, and ready. Check `hyprctl -j plugin list` and `hyprctl -j cua:status` before attributing any behavior to the plugin; a loaded module, a healthy probe, and successful delivery are separate evidence. The background input route admits only the applications and exact versions Cua qualified (Inkscape 1.4.4-6 on stable at the time of writing); for anything else `background_unavailable` is the contract, not a fault to work around. Do not load, unload, or reconfigure the plugin by other means as part of a computer-use task: turning it on enables compositor input, and hot replacing a mapped module is unsupported. After upgrading a loaded plugin, log out and back in; the toggle refuses a loaded module without independent agent keymaps and never unloads it. Older copied toggle flags are refreshed when enabled or loaded at session start to remove their keyboard overrides.
