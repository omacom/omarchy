...
7. **Idle timings are top-level.** `idle.screensaver`, `idle.lock`, and optional `idle.suspend` are seconds since user activity stopped. Suspend is opt-in and is prevented by Stay Awake or the suspend-off toggle. Bar and layout changes may hot-reload, but changes to any idle timing require `omarchy restart shell` afterward.
