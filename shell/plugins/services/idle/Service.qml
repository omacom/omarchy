...
Item {
  ...
  function handleActiveSignal() {
    ...
    // Starting the screensaver makes the compositor report activity, and the
    // main monitor then stays active for the whole display — further pointer
    // motion never re-signals here. Ignore that launch blip (and keep the lock
    // timer armed) while a screensaver window exists or launch grace is still
    // running; dismissMonitor watches for real seat input once the window is up.
