...
| `kiro` | `AmazonCodeWhispererService.GetUsageLimits` (credit pool, same call the `/usage` command makes) | `~/.kiro/sessions/cli/` session sidecars (`user_turn_metadatas` token counts per turn) |
| `fireworks` | Estimated prepaid balance: configured funding minus rated account costs | Fireworks billing API, grouped by day and model for the last 30 days |

Kiro limits need a signed-in CLI; without credentials the panel says so and
falls back to local stats only.
Claude limits need a signed-in CLI; without credentials the panel says so and
falls back to local stats only. A non-default Claude directory is honored via
`CLAUDE_CONFIG_DIR`, Codex via `CODEX_HOME`, Kiro sessions via `KIRO_SESSIONS_DIR`. Fireworks reads
