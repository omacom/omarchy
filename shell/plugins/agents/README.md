...
| `antigravity` | `agy -p "/usage" --output-format json` — the same call the CLI's own `/usage` slash command makes | `~/.gemini/antigravity-cli/history.jsonl` and `conversation_summaries.db` |

Claude limits need a signed-in CLI; without credentials the panel says so and
falls back to local stats only. A non-default Claude directory is honored via
`CLAUDE_CONFIG_DIR`, Codex via `CODEX_HOME`, Antigravity via
`ANTIGRAVITY_DATA_DIR`. Fireworks reads
