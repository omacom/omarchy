...
| `kimi` | Kimi's `/usages` endpoint (weekly summary + rolling windows) | `~/.kimi-code/sessions` `usage.record` transcripts, pi/omp `kimi-coding` messages, opencode sessions on a Kimi provider |
| `grok` | Grok's `billing?format=credits` endpoint (credit pool + on-demand cap) | `~/.grok/sessions` turn transcripts, pi/omp `xai` messages, opencode sessions on an xAI provider |

Claude limits need a signed-in CLI; without credentials the panel says so and
falls back to local stats only. A non-default Claude directory is honored via
`CLAUDE_CONFIG_DIR`, Codex via `CODEX_HOME`, Kimi via `KIMI_CODE_HOME`,
Grok via `GROK_HOME`. Fireworks reads
