...
- **Tokens by day** — one row per day for the last week, with today bolded at the bottom. Local Codex rows append estimated API-equivalent USD cost to the token value; hover a row for component prices, provenance, assumptions, and missing coverage.
- **Tokens by model** — non-Codex providers retain their token-only model rows. All local Codex sources use 30-local-day buckets for one combined token/known-API-cost-estimate table: the four heaviest models followed by Today, 7 days, and 30 days summaries. Window totals include every model even when only four fit on screen. A visible note names excluded missing-price models when known; hover retains detailed priced-token coverage.
