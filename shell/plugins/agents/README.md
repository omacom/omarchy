...
Remote imports read native Codex, Claude, and Kimi JSONL files plus Pi/OMP messages in their usual directories under the connected user's home. Pi/OMP messages are assigned by their recorded provider: `openai-codex` usage belongs to Codex and exact `anthropic` usage belongs to Claude. Claude's `stats-cache.json` and sanitized timestamp/session metadata from `history.jsonl` are used as an aggregate-only fallback when project transcripts have no usage. Aggregate daily/model token totals remain visible, but absent token-category splits keep API-equivalent cost coverage incomplete. Custom source paths and symlinked source trees are not imported.

OpenCode is optional. If `~/.local/share/opencode/opencode.db` is absent, no OpenCode check failure or coverage warning is shown, and native sources, machine management, and All/single-computer views work without an OpenCode binary, prior OpenCode usage, or a `sqlite3` command. If the database exists, the importer uses SSH command execution and only an already installed compatible `sqlite3`. It opens the database with `-readonly`, enables `PRAGMA query_only`, keeps temporary storage in memory, ignores personal SQLite startup commands with `-init /dev/null`, and uses SQLite safe mode when that CLI supports it. SQLite's JSON functions run one fixed projection on the target for session ID, role/provider/model, timestamp, disjoint token counts, and bounded tariff attributes including inference geography; message text, tool data, credentials, and the original JSON column are never returned or cached. The query accepts at most 10,000 result rows, 8 MiB stdout, and the lesser of 15 seconds or the current pass's cooperative time remaining. After a successful query, unchanged database and `opencode.db-wal` size/mtime/mode plus a matching local sanitized-payload digest reuse that result without another SQL invocation. WAL appearance/disappearance or attribute changes force a query, so a committed WAL append that grows the WAL is detected even when its mtime second and the main DB attributes do not move. SFTP v3 exposes whole-second mtimes: a same-size rewrite within that second, or another rewrite preserving every observed DB/WAL attribute, remains unsupported. Unsafe/unreadable WAL metadata leaves the optional portion stale or unknown. Agents writes no remote output/cache file, never invokes `opencode db`, never uses `immutable=1`, and does not copy the database or WAL contents over SFTP. The standard database path and the currently supported `message(session_id, data)` shape are required; custom OpenCode database paths are not detected.

More than 10,000 supported rows or more than 8 MiB projected output is a coverage limit, not an all-time subtotal: no partial result is accepted, so that OpenCode portion stays unknown or retains its prior dated contribution as incomplete. A deterministic row/output-limit result caches only its kind, verified DB/WAL identity and retry deadline locally. The unchanged source is backed off for 24 hours; any observed DB/WAL identity change or expiry retries it immediately on the next pass. Other capability, schema, command, transport and timeout failures are not negative-cached and retry on the next due pass. `refresh --force` bypasses the machine's due schedule but not an unexpired deterministic-limit backoff for an unchanged identity.

A present OpenCode database whose SSH account cannot execute commands, whose `sqlite3` lacks the needed options/JSON functions/schema, or whose live database cannot be opened read-only is a provider-scoped coverage gap. The last dated OpenCode contribution is retained when available; otherwise that optional portion is unknown. Successful native and other-provider contributions remain visible. Normal SQLite reader coordination is not zero-cost: depending on journal mode it can use or create the normal WAL shared-memory index, hold reader locks, delay a checkpoint/WAL reset, or briefly delay a rollback-mode commit. The query bounds limit that exposure but do not promise zero writer impact. No package is installed or requested automatically.
...
Aggregation, pricing and every persistent Agents cache live on the displaying computer. Native and aggregate source metadata is sanitized locally after read-only SFTP transfer; only the optional OpenCode path executes the fixed SQLite JSON projection on the target. The target does not run an Agents helper. Unchanged files reuse the local cache. Transferred log sections can contain conversation text, but only selected usage metadata is retained locally. Original remote mtimes are applied only when completed sanitized copies are promoted so Codex's existing legacy eligibility window is the same locally and remotely. SFTP v3 modification times have whole-second granularity: same-size rewrites within one second, or changes that preserve all checked attributes and sampled fingerprints, are unsupported. Partial final log lines are retried after completion. A malformed complete JSONL record is skipped, cumulatively checkpointed, and marks that source incomplete without permanently hiding later valid records; a wholly invalid replacement retains the last good sanitized metadata. Detected truncations or replacements trigger a replacement scan.

The plugin attempts an import on startup and approximately hourly. Opening the panel or changing tabs does not trigger an import. An explicit refresh requests one immediately. At most two computers import concurrently. Each pass has a 64 MiB SFTP file-payload budget and a cooperative 45-second inventory/transfer limit; the hard SFTP session timeout remains 120 seconds, collectors retain separate 60-second limits, and optional SQL stdout has its distinct 8 MiB bound. A large first import may require several passes. An advancing continuation becomes due after 60 seconds and a no-progress continuation after 300 seconds; the one-minute poll means ordinary automatic starts occur within 60–120 or 300–360 seconds. “Importing” means the snapshot is still incomplete. Source listing, file reads, SSH encryption, SQLite reader coordination and transfer still use resources on the target; caching reduces repeated work but does not make the initial import free.
