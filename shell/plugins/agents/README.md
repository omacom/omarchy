...
| `grok` | SuperGrok weekly credit pool from the CLI billing endpoint, plus Grok Build's share of that week | `$GROK_HOME/sessions` `usage.json` files (default `~/.grok/sessions`) |

Claude limits need a signed-in CLI; without credentials the panel says so and
falls back to local stats only. A non-default Claude directory is honored via
`CLAUDE_CONFIG_DIR`, Codex via `CODEX_HOME`, Grok via `GROK_HOME` (and the CLI OAuth
token in `$GROK_HOME/auth.json` for weekly credits). Fireworks reads
