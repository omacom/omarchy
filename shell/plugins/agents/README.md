...
| `kimi` | Kimi Code platform quotas (5-hour session + weekly) from `api.kimi.com/coding/v1/usages` | `$KIMI_CODE_HOME/sessions` (default `~/.kimi-code/sessions`) `usage.record` events; session-scope running totals are skipped so they are not counted twice |

Claude limits need a signed-in CLI; without credentials the panel says so and
falls back to local stats only. A non-default Claude directory is honored via
`CLAUDE_CONFIG_DIR`, Codex via `CODEX_HOME`, Kimi via `KIMI_CODE_HOME`.
Fireworks reads `FIREWORKS_API_KEY` and `FIREWORKS_ACCOUNT_ID` first, then
`~/.fireworks/auth.ini` (which `firectl set-api-key` creates), then the key
opencode stores in `~/.local/share/opencode/auth.json` when Fireworks is
signed in there. Kimi reads `KIMI_API_KEY` first, then
`~/.kimi-code/credentials/kimi-code.json` (refreshing the OAuth pair the way
the CLI does), then the legacy `~/.kimi` store.
