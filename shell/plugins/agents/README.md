...
prints the record contract (see the `claude`, `codex`, and `grok` collectors in
...
| `grok` | Grok billing (`/v1/billing?format=credits`): weekly credit pool plus the Grok Build slice | Grok CLI `updates.jsonl` `turn_completed` events under `$GROK_HOME/sessions` (default `~/.grok/sessions`) |
