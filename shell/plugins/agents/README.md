...
| `grok` | Grok ACP `_x.ai/billing` (SuperGrok weekly or monthly pool; leftover prepaid credits as a balance). `Money.val` is cents. | `$GROK_HOME/sessions` `updates.jsonl` `turn_completed` rows (plus pi/omp `xai` / `xai-auth` / `xai-oauth` and opencode `providerID=xai`) |
