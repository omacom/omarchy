...
[Muse Code](https://dev.meta.ai) — Meta's `muse` — can be the default agent too, but it isn't one of the preinstalled stubs: picking it installs the `muse-code` package from the Omarchy Package Repository and then walks you through the `muse login` browser flow right in the install window.
