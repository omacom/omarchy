...
The far side starts the agent through a login shell, so a mise-managed agent over there is found the same way it is here, and the session starts in `~/Work` on that machine for the same reason a local one does.

The remote agent also outlives its window. It runs inside a tmux session named for the directory it started in, so closing the terminal, shutting the laptop, or losing the network leaves it working on a machine that is still running fine — and pressing the agent key again drops you back into that same session rather than starting a second agent over the top of the first. Ending it is `omarchy agent stop`, which is the difference between "I'm done with it" and "I closed a window"; with a session name it ends only that one. A machine without tmux installed just runs the agent directly, as before. Omarchy also stops managing the installation once a host is set, because the agent lives on the other machine now: choosing one records the choice and launches it without installing anything locally.
