...
The _Install > AI_ menu also carries a few graphical AI apps: LM Studio Bionic, the ChatGPT desktop app, Grok Bot for chatting with xAI's models, Hermes Desktop, OpenClaw, and the Perplexity desktop app.
...
Omarchy recommends three ways of working with local LLMs: LM Studio, LM Studio Bionic, and Ollama. LM Studio provides a GUI interface for finding open-weight models, installing them, and running them. It's a great way to get going easily. Bionic is a great way to use those models with an agent for coding, research, and document work. Ollama offers a CLI for doing so similarly. But if you're new to local models, I'd start with LM Studio. You can install all three under _Install > AI_ in the Omarchy Menu.
