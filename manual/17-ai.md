...
Built-in harnesses, including [AFK](https://git.mooglest.com/mooglest/afk), appear automatically under _Setup > Defaults > Agent_. Select one there or with `omarchy default agent <id>`; choosing an uninstalled harness installs it first. To add another coding agent as a first-class Omarchy harness — selectable as the default agent and available from that same menu — install its integration with `omarchy plugin add <git-url>`, then choose it with `omarchy default agent <id>`. The integration is available immediately after installation; a harness-only integration has no desktop component to enable. Removing it clears it as your default if it was selected. To wrap a CLI without default-agent integration, run `omarchy-mise-install <package> [command-name]`. The stubs are kept current along with everything else mise manages when you run `omarchy update` (or the `mup` alias).
