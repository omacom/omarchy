...
[Cua Driver](https://cua.ai/docs/cua-driver) lets a coding agent operate the apps on your desktop: it snapshots a window's accessibility tree, acts on the elements it finds there, and verifies the result from fresh state. _Install > AI > Cua_ installs the driver and then offers to install the Cua agent skills, which teach Claude Code, Codex, Hermes, and the other agents the driver's loop, together with an Omarchy companion skill covering the Hyprland and package specifics. Say no and the driver is installed on its own; `cua-driver skills install` adds the skills later. Where your package channel carries the Cua Hyprland plugin, the install adds that too, but never loads it: it is built against one exact Hyprland release, activation is a deliberate per-session step documented with the package, and while it is installed an update that moves Hyprland waits until the plugin is removed. _Remove > AI > Cua_ takes the driver, the plugin, the skills, and the driver's own state back out.
