...
To add a coding agent as a first-class Omarchy harness — selectable as the default agent and available from the Agent menu — install its integration with `omarchy plugin add <git-url>`, then choose it with `omarchy default agent <id>`. To wrap a CLI without default-agent integration, run `omarchy-mise-install <package> [command-name]`. The stubs are kept current along with everything else mise manages when you run `omarchy update` (or the `mup` alias).
