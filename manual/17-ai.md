...
Use _Remove > Agents_ in the Omarchy Menu to uninstall a coding-agent CLI installed through Omarchy's mise launcher, or run `omarchy remove agent codex` with the agent's command name. Only installed agents with an Omarchy-managed launcher appear. Agents installed another way must be removed with their own installer.

After confirmation, removal deletes all mise-managed versions of that agent, including versions used by other projects, and its launcher. Chats, credentials, settings, and project files stay. If it was your default agent, that selection is cleared. Hermes Desktop and OpenClaw use their existing entries under _Remove > AI_.
