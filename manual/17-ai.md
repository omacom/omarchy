...
If you have an NVIDIA RTX, Intel Arc Pro or AMD card, _Install > AI > Local AI_ adds a Local AI mark to the bar. It knows the models that were tested on your exact card and recommends one; a card's Config lists the others. It downloads the model, runs it in a container only your machine can reach, and opens pi, Claude Code, Codex, OpenCode or any other installed coding agent on it in the folder you pick. Two or more free cards of the same kind can run one larger model together. It shows each GPU's memory and temperature, each model's speed, and a history of the tokens your agents used. A card that has no tested model yet shows Coming soon, with the list of supported cards. _Remove > AI > Local AI_ deletes the models, their engines and weights, and its settings.
