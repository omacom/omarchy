...
[Kiro CLI](https://kiro.dev) — AWS's `kiro-cli` — has no mise-manageable release and self-updates in place, so picking it as your default runs its own vendor installer instead of a mise stub.
