...
Omarchy includes launchers for the coding agents below without choosing a default for you. Run a command to install its CLI on first use, then authenticate when prompted:
...
Except for Hermes, these agents use mise's lazy installation: the first launch installs the CLI, and later launches reuse it. Run `omarchy update` to update installed agents with the rest of Omarchy, or `mup` to update mise-managed tools only.

To install all configured lazy tools before you need them, run `mise install --include-lazy`. This includes Omarchy's other default CLI tools as well as the agents. See [Preinstalled CLI tools](18-development-tools.md#preinstalled-cli-tools) for installing a single tool or using your own installation.

Hermes has a separate launcher that installs its CLI with Python 3.13 and uses Hermes Desktop's installation when present. To install it before first use, run:
