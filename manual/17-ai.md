...
[Muse Code](https://dev.meta.ai) — Meta's `muse` — can be the default agent too, but it isn't one of the preinstalled stubs: Meta ships it only as a binary, so picking it installs the [muse-code-bin](https://aur.archlinux.org/packages/muse-code-bin) AUR package and then walks you through the `muse login` browser flow right in the install window.
