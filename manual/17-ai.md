...
The top bar grows an agents icon the first time Omarchy finds AI coding usage on the machine (and stays out of the way until then). The panel behind it tracks every subscription in one place: your plan, the percentage used of the 5-hour session and weekly limits (or the remaining prepaid balance), and token usage by day and by model. Claude Code, Codex, and Fireworks are covered out of the box. A LiteLLM proxy is included once its URL and a virtual key are written to `~/.config/omarchy/agents/litellm.json` (or exported as `LITELLM_BASE_URL` and `LITELLM_API_KEY`). The panel does not ask for the key. The key's owner is whose usage the tab shows, unless that key can read the proxy's global activity.
