...
Omarchy treats AI coding agents as first-class citizens, but it doesn't pick a favorite for you. Instead, every major coding-agent CLI comes pre-wired as a lazy-loaded launcher. [mise](https://mise.jdx.dev/) installs each CLI the first time you run it. Invoke any of these and authenticate when prompted:
...
Except for Hermes, Omarchy declares these as [lazy mise tools](https://mise.jdx.dev/dev-tools/shims.html#lazy-tools) in `/etc/mise/config.toml`, so mise installs the latest version the first time you run them. They do not auto-update on later runs; use `omarchy update` (or the `mup` alias) to update them along with everything else mise manages. Run `mise install --include-lazy` to install every lazy tool up front.

Hermes keeps a custom lazy launcher so it can install with its required Python version and share the Hermes Desktop installation when present. Run `omarchy install hermes cli --now` to install the CLI up front.

To use your own installation instead of a system lazy tool, add its registry name to `[settings] disable_tools` in `~/.config/mise/config.toml`. The default-agent launcher continues to prefer a user-installed Cursor or Muse in `~/.local/bin`.

To install an additional CLI that is not one of Omarchy's defaults, use mise directly with `mise use -g <tool>`.
...
[Muse Code](https://dev.meta.ai) — Meta's `muse` — is installed and updated through mise like the other native lazy tools.
