...
Omarchy declares these as [lazy mise tools](https://mise.jdx.dev/dev-tools/shims.html#lazy-tools) in `/etc/mise/config.toml`, so their commands are available immediately and install on first use. Run `mise install --include-lazy` to install every lazy tool up front. The tools are kept current along with everything else mise manages when you run `omarchy update` (or the `mup` alias).

To wrap an additional CLI that is not one of Omarchy's defaults, run `omarchy-mise-install <package> [command-name]`.
