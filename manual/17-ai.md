...
Left-click the bar icon for the panel, right-click to launch your default agent. Local usage records are regenerated every 15 minutes by `omarchy agent usage-update`. The panel can also include usage from a Linux or macOS computer through your existing SSH access. Its machine list is shared with these commands:

```bash
omarchy agent machine add my-ssh-alias --label Laptop
omarchy agent machine list
omarchy agent machine refresh --force
omarchy agent machine rename MACHINE_ID --label Work
omarchy agent machine remove MACHINE_ID
```

Use the ID printed by `list` for rename and remove. Set up normal SSH access and trust the host key first. Agents installs no helper and writes no Agents-owned output or cache file on the remote computer. Native Codex, Claude, Kimi, and Pi/OMP usage works without OpenCode or an additional `sqlite3` command. OpenCode is only considered when its standard database already exists; a bounded read-only scalar metadata query then requires a compatible existing `sqlite3` and SSH command access. If that optional query cannot run, its last known contribution is retained or its portion is marked unknown while native sources continue. Normal SQLite reader coordination can briefly use or create the database's usual WAL shared-memory bookkeeping and interact with locks/checkpoints, so the bounded query is low-impact rather than zero-impact. Each connection includes only the connected user's usage, not all users on that computer. Copied or synchronized session histories on several computers can be counted more than once; connecting a second SSH alias to the same machine and user is rejected, but copied histories across different computers are not automatically deduplicated.

A missing first import is unknown usage. If a source cannot be read later, its last known contribution is retained where available and coverage is marked incomplete; other readable sources can still update. Removing a machine removes its contribution from the panel without modifying its session files. See the README under `$OMARCHY_PATH/shell/plugins/agents/` for panel settings and the existing synced-folder option.
