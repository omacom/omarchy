...
[Muse Code](https://dev.meta.ai) — Meta's `muse` — can be the default agent too, but it isn't one of the preinstalled stubs: picking it installs Meta's official launcher through mise's HTTP backend and then walks you through the `muse login` browser flow right in the install window. The launcher verifies and updates the native binary for your machine.
