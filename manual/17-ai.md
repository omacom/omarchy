...
The top bar grows an agents icon the first time Omarchy finds AI coding usage on the machine (and stays out of the way until then). The panel behind it tracks every subscription in one place: your plan, the percentage used of the 5-hour session and weekly limits (or the remaining prepaid balance), and token usage by day and by model. Claude Code, Codex, Fireworks, and Grok are covered out of the box. Grok's paid plans share one weekly credit pool across Chat, Imagine, Voice, and Build, so the Grok tab shows that weekly meter and Grok Build's share of it rather than a 5-hour session window.
