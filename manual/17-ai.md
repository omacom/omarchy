...
A crash storm is a different problem from a crash: a test run whose binaries all dump core, or a linker dying once per target, arrives faster than anyone dismisses it, and crash notifications are critical, so none of them expires on its own. The first three crashes of a burst each get their own notification; after that the rest are counted on a single card — "5 more processes crashed" — that updates in place. Click it and it unfolds: the crashes it counted come back as a notification each, so you can pick the one you actually want diagnosed instead of being handed whichever was last. (A card standing for a single crash skips the middle step and diagnoses it directly.) Quiet for a few minutes ends the burst, and the next crash gets a notification of its own again.
