...
| `mmx`      | [MiniMax CLI](https://github.com/MiniMax-AI/cli)                 |
...
`mmx` is MiniMax's official API client rather than a coding harness. Set it up under _Setup > AI > MiniMax_, which exposes login, coding-agent configuration, and Token Plan quota, or run `mmx auth login` directly for the same CLI-owned login flow the other providers use. `mmx agent setup` configures MiniMax in Codex, Claude Code, OpenCode, Pi, or another supported harness, and its Token Plan quota appears automatically in the agents panel.
