...
| `phantombot` | [Phantombot](https://phantombot.bot), a persona-first assistant that armors any harness underneath |
...
Phantombot is the personal-assistant shape of the same idea: instead of one more harness, it's a persistent persona (memory, channels, scheduled tasks) that runs Pi, Claude Code, or Codex underneath and keeps their context, credentials, and history in one place. Picking it as the default installs its checksum-verified installer, which also registers a systemd user service so the persona stays reachable from Telegram or PhantomChat between terminal sessions. The chat TUI opens with bare `phantombot`; `Super + Shift + Ctrl + A` still launches your default agent either way.
