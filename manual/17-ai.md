...
Omarchy recommends three ways of working with local LLMs: LM Studio, LM Studio Bionic, and Ollama. LM Studio provides a GUI for finding, installing, and running open-weight models. Bionic is a great way to use those models with an agent for coding, research, and document work. Ollama offers a similar workflow from the command line. You can install all three under _Install > AI_ in the Omarchy Menu.
