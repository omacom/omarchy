...
Omarchy treats AI coding agents as first-class citizens, but it doesn't pick a favorite for you. Instead, all but one of the major coding-agent CLIs come pre-wired as lazy-loaded launchers. The launchers are tiny [mise](https://mise.jdx.dev/)-managed stubs in `~/.local/bin/`, so nothing is downloaded until the first time you actually run one. Invoke any of these and authenticate when prompted:
...
`hermes` is the other exception: it is not a mise stub. Choosing it installs Hermes the way upstream does, as a runtime under `~/.hermes` that updates itself; the Desktop apps section below has the details.
...
Hermes is the one to know about, because there is only ever one Hermes for a user, and Omarchy installs it the way upstream does: a checkout under `~/.hermes` with its own environment, which `hermes update` and the desktop app's own updater move forward from then on. Choosing Hermes as the default agent sets that runtime up, which takes a few minutes and shows its progress in a terminal; Hermes Desktop sets up the same runtime and lays its app on top, so the terminal `hermes` command, the default agent and the app are the same installation whichever order you installed them in. Installing the app also hands Hermes the Omarchy theme as a skin named `omarchy`, which every Hermes surface follows as you switch themes; pick another under Hermes' Appearance settings or with `/skin` if you'd rather it didn't, and Omarchy leaves that choice alone. Removing Hermes under _Remove > AI_ takes the app and that runtime with it, and keeps your chats, memories, and the skills Hermes wrote for itself unless you tell it otherwise: it asks, defaulting to no, whether that data and your connection settings should go too.
