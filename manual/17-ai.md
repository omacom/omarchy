...
[Cua Driver](https://cua.ai/docs/cua-driver) lets a coding agent operate the apps on your desktop: it snapshots a window's accessibility tree, acts on the elements it finds there, and verifies the result from fresh state. _Install > AI > Cua_ installs the driver and then offers to install the Cua agent skills, which teach Claude Code, Codex, Hermes, and the other agents the driver's loop, together with an Omarchy companion skill covering the Hyprland and package specifics. Say no and the driver is installed on its own; `cua-driver skills install` adds the skills later. Where your package channel carries the Cua Hyprland plugin, the install adds that too, but never loads it. _Trigger > Toggle > Cua Input_ (or `omarchy toggle cua-input`) runs the package's compatibility check, loads the plugin, and switches on its input route; plugin 0.26.1-5 or newer keeps your keyboard layout and remaps unchanged while its background agent keyboards use their own US keymaps. Num Lock stays unchanged and does not block keys whose meaning it leaves unaffected. Foreground typing can still refuse incompatible keys or shortcuts, Caps Lock, held modifiers, or unsupported layout groups. After a plugin update, log out and back in before enabling it. The plugin is built against one exact Hyprland release, and while it is installed an update that moves Hyprland waits until the plugin is removed. _Remove > AI > Cua_ takes the driver, the plugin, the skills, and the driver's own state back out.
