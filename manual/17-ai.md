...
[Muse Code](https://dev.meta.ai) — Meta's `muse` — uses a preinstalled mise stub like the other agents. Picking it as the default installs Meta's official launcher through mise's HTTP backend and then walks you through the `muse login` browser flow right in the install window. The launcher verifies and updates the native binary for your machine.
