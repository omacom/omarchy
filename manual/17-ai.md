...
The watching is on by default. Turn it off under _Trigger > Toggle > Crash Capture_ (or with `omarchy toggle crash-capture`) and the notifications stop; `omarchy agent crash <pid>` still works by hand. One death isn't announced: a process that had core dumps off and died of a signal carrying no fault cause, which is what a test suite's stub does with `ulimit -c 0; kill -SEGV $$`. There's no core to read. A program that faults directly with cores off still notifies, and so does a `kill -ABRT` you send a hung program yourself — unless that program had zeroed its own core limit, as ssh-agent and gpg-agent do to keep keys out of a core. Either way it's still in `coredumpctl list` for `omarchy agent crash <pid>`.
