...
| `commandcode`  | [CommandCode](https://commandcode.ai/docs), the coding agent that learns your taste |
...
CommandCode reads screenshots three ways — pasting, dragging into the terminal, or naming a file path — and every model gets vision through a side-call that is enabled on first use in `/config`. Binary paste needs terminal graphics support, which the default terminal lacks, so the reliable route is by path: open clipboard history (`Super + Ctrl + V`) with the terminal focused, pick the screenshot, and its path is typed for you.

Two more AI tools are available on demand under _Install > AI_: the [Qwen Audio Agent](https://github.com/QwenAudio/qwen-audio-agent) (`qwenaudio`), a realtime voice runtime that keeps a coding agent talking while it works, and [Colibrì](https://github.com/JustVugg/colibri) (`coli`), a pure-C MoE inference engine that runs very large models on hardware you already own. They are not pre-wired as lazy launchers; install them from the menu and they appear on PATH.
