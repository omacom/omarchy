...
| `devin`    | [Devin CLI](https://devin.ai/cli) from Cognition                 |
...
`devin` is the other exception: it ships its own installer (`curl -fsSL https://cli.devin.ai/install.sh | bash`) rather than going through mise, so picking it as the default agent runs that installer directly instead of creating a mise stub.
