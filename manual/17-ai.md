...
The top bar grows an agents icon the first time Omarchy finds AI coding usage on the machine (and stays out of the way until then). The panel behind it tracks every subscription in one place: your plan, the percentage used of the 5-hour session and weekly limits (or the remaining prepaid balance), and token usage by day and by model. An **All** tab sums every harness, and Day / Week / Month / Total filters the charts without waiting for a quota reset. Claude Code, Codex, Fireworks, Antigravity, Hermes, Grok, Cursor, OpenCode, and Devin are covered out of the box. Cursor's meters follow the same Cursor Models / Other Models sentences that Settings shows. Day never paints leftover `today*` totals from a file that stopped being rewritten, and bounded periods never fall back to all-time or billing-cycle model maps.
