...
Use the ID printed by `list` for rename and remove. Set up normal SSH access and trust the host key first. Nothing is installed or persistently written on the remote computer. Native Codex and Claude usage works without OpenCode or an additional `sqlite3` command. Each connection includes only the connected user's usage, not all users on that computer. Copied or synchronized session histories on several computers can be counted more than once; connecting a second SSH alias to the same machine and user is rejected, but copied histories across different computers are not automatically deduplicated.
