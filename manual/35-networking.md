...
That panel scans for Wi-Fi networks, shows signal strength, and connects. The eye beside a passphrase field, or `Alt + S`, toggles its visibility. Ethernet needs nothing at all — plug it in and it works. If you'd rather stay in the terminal, `nmtui` gives you the same controls, and there's an `omarchy network` command group too.
