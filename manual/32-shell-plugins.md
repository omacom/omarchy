...
If you've read a specific commit and want exactly that one, `--commit <sha>` installs it and pins the plugin there. Nobody else has checked that commit, so the pin is only as good as your own review.
...
Updating fast-forwards that same checkout, and it stays on the track you added it with. A plugin added at a verified snapshot moves to the marketplace's newest verified snapshot. One added from upstream HEAD — with `--head`, because it wasn't listed, or because you added it before verified installs existed — pulls upstream HEAD like always. A plugin added with `--commit` stays pinned where it is:
