...
Build your UI from the shell's shared components (`Button`, `Toggle`, `BarIconButton` and friends) so supported Quickshell builds can expose it to screen readers. The one thing they can't work out on their own is what an icon means, so give every icon-only control a name with `accessibleName: "Refresh"` (or a `tooltipText`, which also shows a tooltip). The [shell reference](https://github.com/omacom/omarchy/blob/quattro/docs/omarchy-shell.md#accessibility) has the compatibility details.
