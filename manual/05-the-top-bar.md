...
The same file also holds your idle timings at the top level, outside the `bar` key: `idle.screensaver` and `idle.lock`, both in seconds since you went idle. So the default screensaver kicks in at 150 seconds and the lock at 300. Bar and layout changes may hot-reload, but after changing any idle timing, always run `omarchy restart shell`.
