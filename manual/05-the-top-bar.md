...
- **Power** shows battery stats, switches power profiles (it remembers a separate choice for battery and AC), and prints some system info. On laptops whose battery supports charge limiting, it also offers a charge-limit picker — cap charging at 80% or 90% instead of 100% to spare the battery when you live on the charger. Where supported, charging resumes below five percentage points under the selected maximum, avoiding repeated top-ups after small drops. On hardware using the LG laptop charge control, only 80% and 100% are offered. Other hardware may reject individual presets; a rejected change restores the previous limits. The choice persists across reboots.
