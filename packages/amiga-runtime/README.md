...
The package smoke is not a demo rendering or audible M test. An offscreen Qt platform has no Quickshell PanelWindow backend, so complete Overlay/menu presentation needs a separately authorized isolated Wayland desktop. Each session shuffles the eligible demos without repeats until the whole list has played. Validate destination state restoration and automatic rotation independently: a visible owned emulator exit advances immediately, a recoverable per-demo failure advances to the next entry, and a 600-second maximum prevents a stuck demo holding the session indefinitely. Re-run audible M tests with the finished package's exact Pulse stream, not a default sink or microphone capture.
