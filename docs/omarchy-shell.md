...
New Git installations default to Ward. Content without a sandbox declaration gets a best-effort restricted worker with no resource permissions; unsupported kinds or dependencies may prevent loading. `omarchy plugin add <source> --yolo` selects all declared permissions for sandbox-native code in the same worker, but explicitly trusts legacy code without a declaration to run in-process. `--trusted-local` accepts only local Git folders and follows the same declaration split. `--yes` alone never selects either mode. Both sandbox-native modes need Ward; legacy YOLO has arbitrary same-account authority and its facade is not containment. First-party plugins remain unchanged. New plugins land disabled unless explicitly enabled. `--inspect --json` validates without installation; `--commit <sha>` binds the inspected commit.
...
The [shared plugin runtime](portable-plugin-runtime.md) lets a sandbox-native plugin keep one source revision for normal Ward and explicit YOLO. Every entry declares `required property var runtime`, supplied before initialization. It provides owned local jobs, private paths, named host commands and operation helpers; plugin code needs neither a YOLO branch nor QML process wrappers.
