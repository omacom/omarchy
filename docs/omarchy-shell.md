...
Panels, overlays, and menus are loaded when summoned. Plugins can set the top-level manifest key `keepLoaded: true` to survive between summons, and to keep a service mounted across plugin hot-reload (so `omarchy.lock` is not destroyed while Hyprland still holds the session lock). A kept service instance is reused only while its resolved entry-point path and trust remain unchanged; edits to the same service file still take effect on a shell restart. First-party services are loaded at startup.
...
The `omarchy.*` namespace remains reserved for trusted built-ins from `$OMARCHY_PATH`; home and XDG data directories cannot supply those IDs. Plugins named `omacom.*` are also trusted when discovered through `$OMARCHY_PATH` or `$XDG_DATA_DIRS`. A personal `omacom.*` copy still takes precedence, but remains third-party and cannot grant itself host capabilities. All other IDs remain third-party regardless of location. Trusted plugins receive the first-party host APIs and manifest-declared capabilities. They are enabled by default unless explicitly disabled, except full-bar plugins which must be selected; placing a widget on the bar is still a separate action. After installing a package, rescan with `omarchy-shell shell rescanPlugins` and enable or place the desired plugin normally.
