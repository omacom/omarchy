...
For one source revision that also runs in explicit YOLO mode, use the [shared plugin runtime](portable-plugin-runtime.md). Entry points declare `required property var runtime`, injected before initialization. QML uses owned local jobs and named `runtime.exec` calls; worker-local scripts use `/bootstrap` directly. Sandbox-native YOLO selects all declared permissions without changing this worker boundary or bypassing admission. Failure never selects trusted in-process execution.

A plugin is a Git repository containing `manifest.json`, QML and its scripts/assets. Use `sandbox` to declare the resources needed inside isolation; omitting it grants none. Review snapshots the repository; approval binds that revision. Later checkout edits do not change approved code. There are no install scripts, build hooks, post-approval hooks or manifest-driven data-copy hooks. Include already-built assets before review.
