...
| `omarchy-update-aur-pkgs` | Reports pending AUR updates, holds them during unattended/non-terminal runs, and opens complete recipes and available build-file diffs in a blocking viewer before final confirmation during an interactive update. Query failures and declined or broken optional AUR transactions warn without aborting the system update. | **Keep during the compatibility phase.** This is an emergency review gate, not the isolated and attested pipeline described in [`aur-security.md`](aur-security.md). |
