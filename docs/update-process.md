...
| `$XDG_RUNTIME_DIR/omarchy-update.log` | user | Transcript of `omarchy update`, used by `omarchy-update-analyze-logs`. The path is exported as `OMARCHY_UPDATE_LOG`; it lives in the per-user runtime directory rather than `/tmp` so another local user cannot pre-create it and block the update. |
...
```text
omarchy-update
  ├─ refuse to run as root (every step sudo's for itself)
  ├─ ensure transcript logging through script(1) → $OMARCHY_UPDATE_LOG
  ├─ omarchy-update-lock
...
- A failure should leave enough output in `$OMARCHY_UPDATE_LOG` and the
...
| `omarchy-update-analyze-logs` | Scans `$OMARCHY_UPDATE_LOG` (falling back to `$XDG_RUNTIME_DIR/omarchy-update.log`) for known failure patterns, currently initramfs generation. Exits quietly when no transcript exists. | **Keep/expand.** Useful safety net; should grow only for high-signal checks. |
