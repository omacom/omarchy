...
A first-party theme can pair a still background with a one-shot boot intro. Aim for a video between five and seven seconds long that opens dark or fades in from black, since the screen is black just before it starts, and ends on the still image. OWE holds the final frame while the shell's still takes back the background layer, so a video that ends on the still hands off without a visible jump. Until OWE starts the intro, the shell fills the screen with black instead of the still, so the still never shows before its intro. For `backgrounds/0-winding-road.webp`, place the video at `intros/0-winding-road.mp4` and the still image's SHA-256 at `intros/0-winding-road.sha256`. The hash binding prevents a same-named user background from receiving a mismatched packaged intro. User-managed intros are stored under `~/.config/omarchy/backgrounds/<name>/intros/<background-sha256>/` and take precedence over first-party intros without colliding when multiple backgrounds share a filename stem.

The shell runs the resolver under a lock and records the boot before calling `owe intro`, so a shell restart during playback cannot replay the intro. If `owe intro` fails before OWE has accepted the intro (its `intro_result` status is still empty), the boot is released for a bounded retry of up to 60 attempts, one second apart. An intro that OWE accepted and then interrupted, for example by a screen lock or background change, still consumes the boot.
