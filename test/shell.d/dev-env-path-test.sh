...
case ":$default_path:" in
  *":$home/.ghcup/bin:"*) fail "env-bootstrap skips GHCup bin when it is absent" "PATH: $default_path" ;;
esac
pass "env-bootstrap skips GHCup bin when it is absent"
assert_path_first "$default_path" "$tmpdir/unrelated/bin" "env-bootstrap appends user-level paths after existing entries"

mkdir -p "$home/.ghcup/bin"
printf 'touch "%s/PWNED"\nexport PATH="%s/.ghcup/bin:$PATH"\n' "$tmpdir" "$home" >"$home/.ghcup/env"
mapfile -t ghcup_result < <(run_bootstrap bash "$bootstrap" "$home" "$tmpdir/unrelated/bin:/usr/bin")
ghcup_path=${ghcup_result[1]}
assert_path_present "$ghcup_path" "$home/.ghcup/bin" "env-bootstrap appends ~/.ghcup/bin when that directory exists"
[[ ! -e $tmpdir/PWNED ]] || fail "env-bootstrap does not source ~/.ghcup/env"
pass "env-bootstrap does not source ~/.ghcup/env"
rm -rf "$home/.ghcup"
