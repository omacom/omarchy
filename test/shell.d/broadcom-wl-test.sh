...
grep -qx 'linux-headers' "$other_packages" ||
  fail "the ISO caches the matching kernel headers"
pass "the Broadcom wl setup and its packages are available during offline installation"
...
for wifi_id in 43a0 43b1 4331; do
  run_leaf "$wifi_id"
  grep -Fxq 'broadcom-wl-dkms linux-headers' "$calls" ||
    fail "a supported Broadcom adapter installs wl with matching headers" "14e4:$wifi_id"
done
pass "BCM4360, BCM4352, and BCM4331 install wl with matching headers"
