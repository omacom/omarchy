...
assertDeepEqual(
  network.parseWifiQr('WIFI:T:WPA;S:Cafe\\;Guest\\\\5G;P:p\\,a\\:ss\\;word\\\\42;;'),
  { ssid: 'Cafe;Guest\\5G', password: 'p,a:ss;word\\42', security: 'WPA', hidden: false },
  'network parses the escaped payload generated by its Wi-Fi QR helper'
)
assertDeepEqual(
  network.parseWifiQr('WIFI:H:true;P:"correct horse";S:"Hidden Cafe";T:WPA3;;'),
  { ssid: 'Hidden Cafe', password: 'correct horse', security: 'SAE', hidden: true },
  'network parses quoted, reordered hidden WPA3 fields'
)
assertDeepEqual(
  network.parseWifiQr('WIFI:S:\\"Quoted\\";T:nopass;;'),
  { ssid: '"Quoted"', password: '', security: 'nopass', hidden: false },
  'network preserves escaped quotes as part of an open SSID'
)
assertDeepEqual(
  network.parseWifiQr('WIFI:S:Open Cafe;;'),
  { ssid: 'Open Cafe', password: '', security: 'nopass', hidden: false },
  'network treats an omitted security field as open Wi-Fi'
)
assertDeepEqual(
  network.parseWifiQr('WIFI:T:WEP;S:Legacy;P:"abcde";;'),
  { ssid: 'Legacy', password: 'abcde', security: 'WEP', hidden: false },
  'network decodes a quoted ASCII WEP key'
)

function wifiQrError(payload) {
  try {
    network.parseWifiQr(payload)
  } catch (error) {
...
assertEqual(wifiQrError('https://example.com'), 'That QR code does not contain Wi-Fi credentials', 'network rejects non-Wi-Fi QR codes')
assertEqual(wifiQrError('WIFI:T:WPA;P:secret;;'), 'Wi-Fi QR code has no network name', 'network requires a QR SSID')
assertEqual(wifiQrError('WIFI:T:WPA;S:Cafe;;'), 'Wi-Fi QR code has no password', 'network requires personal Wi-Fi credentials')
assertEqual(wifiQrError('WIFI:T:WPA2-EAP;S:Office;P:secret;;'), 'Enterprise Wi-Fi QR codes are not supported', 'network rejects enterprise QR profiles clearly')
assertEqual(wifiQrError('WIFI:T:WPA;S:Cafe\\q;P:secret;;'), 'Invalid Wi-Fi QR code', 'network rejects malformed QR escaping')
