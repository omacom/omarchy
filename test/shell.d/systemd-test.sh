...
soundwire_service="$ROOT/default/systemd/user/omarchy-amd-soundwire-resume.service"
grep -Fx 'ExecStart=/usr/bin/omarchy-hw-recover-amd-soundwire' "$soundwire_service" >/dev/null
grep -Fx 'ConditionPathExists=/proc/asound/amdsoundwire' "$soundwire_service" >/dev/null
grep -F 'omarchy-amd-soundwire-resume.service' "$first_run_units" >/dev/null
grep -rlF 'enable --now omarchy-amd-soundwire-resume.service' "$ROOT/migrations" >/dev/null
pass "AMD SoundWire recovery only runs on affected hardware"
