...
# Dual-battery ThinkPad whose internal pack (BAT0) is dead: no voltage, no
# stored energy, enumerated first by UPower. The removable pack (BAT1) powers
# the machine. DisplayDevice itself is correct here (dead pack advertises 0).
mkdir -p "$tmp_dir/bin"
...
# peripheral: it must not leak into cycle counts.
...
# Both packs alive: each pack contributes its own cycle count, in sysfs order.
mkdir -p "$tmp_dir/power2/BAT0" "$tmp_dir/power2/BAT1"

printf 'Battery\n' >"$tmp_dir/power2/BAT0/type"
printf '1\n' >"$tmp_dir/power2/BAT0/present"
printf '12000000\n' >"$tmp_dir/power2/BAT0/voltage_now"
...
pass "dual-battery machine with two live packs lists both cycle counts"
...
# near full reads as 49% with a ten-hour charge (#7916). Headline figures must
# follow the packs that actually hold a charge — and a dead pack still
