...
assert(/modelData\.chipName \|\| modelData\.providerName/.test(panelSource), 'agents panel uses short chip names when many harnesses are present')
JS
...
assert(/function chipNameFor\(id, name\)/.test(mainSource), 'agents display shortens Antigravity to AGI on the chip row')
assert(/if \(id === "antigravity"\) return "AGI"/.test(mainSource), 'agents display labels Antigravity as AGI')
