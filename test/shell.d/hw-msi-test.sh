...
grep -q "MSI_COOLER_BOOST_ON_AT=85" "$ROOT/default/systemd/system/omarchy-msi-cooler-boost-watch.service" || fail "ON_AT=85 not found"

pass "unit off-threshold is 70"
grep -q "MSI_COOLER_BOOST_OFF_AT=70" "$ROOT/default/systemd/system/omarchy-msi-cooler-boost-watch.service" || fail "OFF_AT=70 not found"

pass "unit sustain is 30"
grep -q "MSI_COOLER_BOOST_SUSTAIN=30" "$ROOT/default/systemd/system/omarchy-msi-cooler-boost-watch.service" || fail "SUSTAIN=30 not found"

pass "unit interval is 5"
grep -q "MSI_COOLER_BOOST_INTERVAL=5" "$ROOT/default/systemd/system/omarchy-msi-cooler-boost-watch.service" || fail "INTERVAL=5 not found"
