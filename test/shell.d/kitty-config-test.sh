...
run_command() {
  env HOME="$test_home" OMARCHY_PATH="$ROOT" GDK_SCALE=1 PATH="$test_dir/bin:$ROOT/bin:$PATH" "$ROOT/bin/$@"
}
...
grep -qx 'font_size 12' "$kitty_config" || fail "size command creates size override"
...
grep -qx 'font_size 9' "$kitty_config" || fail "size reset restores default"
...
grep -qx 'font_size 12' "$kitty_config" || fail "size command handles absent setting"
! grep -q '^include ' "$kitty_config" || fail "font controls must not opt users back into theming"
rm "$kitty_config"
run_command omarchy-display-text-size 16
grep -qx 'font_size 12' "$kitty_config" || fail "size command handles absent config"
