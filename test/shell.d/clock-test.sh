...
assert(/contentWidth: panel\.fittedContentWidth\(Math\.max\(Style\.space\(560\), heroRow\.implicitWidth \+ panel\.padding \* 2 \+ Border\.left\(panel\.borderSpec\) \+ Border\.right\(panel\.borderSpec\)\)\)/.test(panelSource), 'calendar fits the hero plus padding and borders within the screen limit')
assert(/width: Math\.max\(calendarScroll\.width, gridColumn\.width, heroRow\.implicitWidth\)/.test(panelSource), 'calendar scrolls rather than clipping the grid or hero on a narrow popup')
