...
assert(/function close\(\) \{[\s\S]*?root\.controller\.hide\(\)[\s\S]*?setCenterHoverRevealSuppressed\(false\)/.test(panelSource), 'calendar hides the overlay before releasing the hover-reveal flag')
assert(!/function close\(\) \{\s*\n\s*setCenterHoverRevealSuppressed\(false\)/.test(panelSource), 'calendar does not write the hover-reveal flag before hide')
