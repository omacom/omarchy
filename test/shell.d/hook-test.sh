...
# A path whose basename is `.` or `..` is a directory, so the -f check would
# also reject it — but the name guard must fire first so the error is specific.
mkdir -p "$tmpdir/as-dot"
if HOME="$home" "$ROOT/bin/omarchy-hook-install" post-update "$tmpdir/as-dot/." >"$tmpdir/out" 2>"$tmpdir/err"; then
  fail "hook-install rejects a file basename of ."
fi
