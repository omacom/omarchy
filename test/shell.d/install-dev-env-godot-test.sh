...
run_godot() {
  ...
  HOME="$home" \
    ...
    "$ROOT/bin/omarchy-$action-development-godot" "$@" </dev/null
}
...
grep -Fq 'omarchy-godot-lsp: managed by omarchy-install-development-godot' "$(nvim_lsp_file)" ||
...
if PATH="$stub_bin:$ROOT/bin" HOME="$home" OMARCHY_PATH="$ROOT" \
  "$ROOT/bin/omarchy-install-development-godot" zed >"$test_tmp/install-zed.out" 2>"$test_tmp/install-zed.err" </dev/null; then
...
if command -v script >/dev/null; then
  ...
  PATH="$stub_bin:$ROOT/bin:$PATH" \
    ...
    OMARCHY_TEST_LOG="$log_file" \
    OMARCHY_TEST_STUB_BIN="$stub_bin" \
    script -qefc "$ROOT/bin/omarchy-install-development-godot" /dev/null \
...
grep -Fq 'prefer_wayland' "$ROOT/bin/omarchy-install-development-godot" ||
...
grep -Fxq $'pkg-add\tgodot' "$log_file" || fail "install-dev-env godot calls omarchy-install-development-godot" "$(cat "$log_file")"
grep -Fq 'text_editor/external/use_external_editor = false' "$(settings_file)" ||
  fail "install-dev-env godot keeps the built-in editor when non-interactive" "$(cat "$(settings_file)")"
pass "install-dev-env godot delegates to omarchy-install-development-godot"
