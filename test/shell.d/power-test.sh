...
assert(!power.chargeThresholdActive({ isPresent: true, percentage: 0.11, state: states.Charging, changeRate: 3.0, timeToFull: 9 * 60 * 60 }, false, states), 'power does not treat a long UPower time-to-full as a charge threshold')
assertEqual(power.batteryFraction({ isPresent: true, percentage: 0.106 }, { percentage: '99%' }), 0.99, 'power prefers status-script percentage over UPower')
assertEqual(power.modeLabel({ isPresent: true, percentage: 0.106, state: states.Charging }, false, states, { percentage: '99%', state: 'fully-charged' }), 'Fully charged', 'power labels full from status script')
assert(!power.chargeThresholdActive({ isPresent: true, percentage: 0.106, state: states.Charging, timeToFull: 9 * 60 * 60 }, false, states, { percentage: '99%', state: 'charging' }), 'power does not hold when status script says charging')
