...
EOF
  ...
  if [[ ${DDC_WRITE_FAIL:-0} == "1" || ( ${DDC_FAST_WRITE_FAIL:-0} == "1" && $* != *" --max-setvcp-and-verify-tries 2 "* ) ]]; then
    [[ $* == *" --noverify "* ]] || exit 1
  fi
fi
SH
...
grep -F 'ddcutil --bus 7 --skip-ddc-checks --verify --max-setvcp-and-verify-tries 2 setvcp 10 20' "$call_log" >/dev/null || \
  fail "the first external write verifies and retries the fast DDC path"
pass "the first external write verifies and retries the fast DDC path"
...
grep -F 'ddcutil --bus 7 --skip-ddc-checks --verify --max-setvcp-and-verify-tries 2 setvcp 10 24' "$call_log" >/dev/null || \
...
DDC_FAST_WRITE_FAIL=1 run_brightness --no-osd --monitor DP-1 35% || \
  fail "a rejected fast DDC write is retried in the same process"
(( $(grep -c ' setvcp 10 ' "$call_log") == set_count + 1 )) || \
  fail "external brightness uses one DDC process"
pass "external brightness retries in the same DDC process"
...
(( $(grep -c ' setvcp 10 ' "$call_log") == set_count + 1 )) || \
  fail "a rejected write uses one DDC process"
get_count=$(grep -c ' getvcp 10 ' "$call_log")
...
grep -F 'ddcutil --bus 7 --skip-ddc-checks --verify --max-setvcp-and-verify-tries 2 setvcp 10 50' "$call_log" >/dev/null || \
...
grep -F 'ddcutil --bus 7 --skip-ddc-checks --verify --max-setvcp-and-verify-tries 2 setvcp 10 5' "$call_log" >/dev/null || \
