...
# Case 4: Migration moves legacy pure Omarchy fonts.conf to dropin if dropin does not exist
...
# Case 5: Migration leaves custom user fonts.conf untouched
printf '%s\n' "$custom_rule" >"$user_fonts_conf"
run_migration >/dev/null
...
# Case 6: Migration idempotency
run_migration >/dev/null
[[ $(cat "$user_fonts_conf") == "$custom_rule" ]] || fail "migration rerun modified custom fonts.conf"
