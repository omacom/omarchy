...
import json, os, sys
args = sys.argv[1:]
i = args.index('susssasa{sv}i')
assert args[i + 4] == 'Sign in to ' + os.environ['STUB_LABEL'], args
assert args[i + 5] == 'This network requires sign-in. Click to open the login page.', args
j = args.index('omarchy-exec-argv')
assert json.loads(args[j + 2]) == ['omarchy-network-portal', '--open'], args
...
url=$(run_portal --url)
[[ $url == "http://ping.archlinux.org/nm-check.txt" ]] || fail "--url returns the splash page from the redirect" "$url"
pass "--url uses the fixed HTTP sign-in entry point"
...
[[ $url == "http://ping.archlinux.org/nm-check.txt" ]] || fail "--url stays fixed when no redirect exists" "$url"
pass "a WISPr payload counts as a portal without becoming the browser target"

: >"$STUB_BROWSER_LOG"
run_portal --open >/dev/null
[[ $(cat "$STUB_BROWSER_LOG") == "http://ping.archlinux.org/nm-check.txt" ]] || fail "--open hands the fixed URL to the default browser" "$(cat "$STUB_BROWSER_LOG")"
pass "--open opens the fixed URL in the default browser"

# A portal-controlled redirect must never become a browser option or URI scheme.
for target in '--load-extension=/tmp/portal' 'file:///etc/passwd' 'javascript:alert(1)' '/relative-login'; do
  export STUB_RESPONSE="HTTP/1.1 302 Found
Location: $target
"
  : >"$STUB_BROWSER_LOG"
  run_portal --open >/dev/null
  [[ $(cat "$STUB_BROWSER_LOG") == "http://ping.archlinux.org/nm-check.txt" ]] || fail "ignores portal-controlled browser targets" "$target"
done
pass "portal redirects cannot supply browser arguments"
...
#!/bin/bash
exec "$ROOT/bin/omarchy-notification-send" "$@"
STUB
