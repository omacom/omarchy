...
assertDeepEqual(
  ...
  'monitor toggle command disables external monitor via hyprctl eval'
)
assertDeepEqual(
  monitor.monitorToggleCommand('DP-2', false, 'eDP-1'),
  ['hyprctl', 'eval', 'hl.monitor({ output = "DP-2", mode = "preferred", position = "auto", disabled = false })'],
  'monitor toggle command enables external monitor via hyprctl eval with preferred mode and auto position'
)
assertEqual(
