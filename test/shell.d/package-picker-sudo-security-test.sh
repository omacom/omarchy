...
case ${1:-} in
  ...
  -k)
    ...
    printf 'revoke\n' >>"$TEST_REVOKE_COUNT"
    printf 'SUDO:revoke\n' >>"$TEST_EVENT_LOG"
    if [[ ${TEST_INITIAL_REVOKE_FAIL:-0} == 1 ]] && (( count == 0 )); then exit 90; fi
...
for command in install remove; do
  run_picker "$command" 1 TEST_INITIAL_REVOKE_FAIL=1
  ! grep -Eq '^(QUERY|FZF|SUDO:transaction)$' "$event_log" || fail "$command continued after initial revocation failed"
  ...
  grep -Fxq 'DONE:0' "$event_log" || fail "$command did not present transaction success"
  ...
  grep -Fxq 'DONE:1' "$event_log" || fail "$command did not present authentication failure"
  [[ $(tail -n 1 "$event_log") == "SUDO:revoke" ]] || fail "$command skipped cleanup after authentication failure"
  run_picker "$command" 77 TEST_TRANSACTION_FAIL=1
  grep -Fxq 'DONE:77' "$event_log" || fail "$command did not present transaction failure"
  [[ $(tail -n 1 "$event_log") == "SUDO:revoke" ]] || fail "$command skipped cleanup after transaction failure"
...
for command in install remove; do
  ...
  run_picker "$command" 0 \
    "BASH_FUNC_cat%%=() { : > '$startup_marker'; /usr/bin/cat \"\$@\"; }" \
    TEST_PICKER_RESULT=empty
  [[ ! -e $startup_marker ]] || fail "$command picker passed an exported function to child Bash"
done
